...
for n in range(total_events):
    ...
    line_segments = LineCollection(combined, colors = 'blue')
    ...
    ax.scatter(full_df.y_calc[(full_df.event == n)]/10, 
                full_df.z_calc[(full_df.event == n)]/10, s =.1, marker = '.', color = 'orange', 
                label = 'Calculated Positions')
    ax.scatter(reconstruction_df.Y_Position[(reconstruction_df.event == n)]/10, 
            reconstruction_df.Z_Position[(reconstruction_df.event == n)]/10, s = 10, color = 'red', 
            label = 'nResets >= 8')
